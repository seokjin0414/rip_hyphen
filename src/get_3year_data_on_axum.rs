...
use std::{env, process::{Child, Command}, sync::Arc};
use axum::{
    extract::State,
    http::StatusCode,
...
use crate::{
    models::handler_models::crawling_models::{
        kepco_models::{
            KepcoRequestBody, KepcoData, MetaResponseData, ThreeYearKepcoDataResponse,
        },
    },
    server_init::get_state::ServerState,
};
...
pub async fn get_3year_kepco_data_of_handler(
    ...
    // driver path
    let chromedriver_path = "/usr/bin/chromedriver";
    ...
    let mut chromedriver_process = match Command::new(chromedriver_path)
        ...
        .spawn() {
        Ok(process) => process,
        Err(e) => return error_response("Could not start chromedriver process", e), // 에러 메시지 단순화
    };
    ...
    let capabilities: Map<String, Value> = match serde_json::from_value(json!({
        ...
        Ok(result) => result,
        Err(e) => return error_response("Could not set Chrome capabilities", e), // 에러 메시지 단순화
    };
    ...
    // 브라우저 창 크기 설정
    if let Err(e) = client_arc.set_window_rect(0, 0, 774, 857).await {
        return error_response("Failed to set view size", e); // 에러 메시지 단순화
    }

    // 페이지 이동
    if let Err(e) = client_arc.goto("https://online.kepco.co.kr").await {
        return error_response("Failed to navigate to URL", e); // 에러 메시지 단순화
    }

    // 여러 함수 호출 시 에러 처리를 단순화
    handle_navigation_and_click(&client_arc, &mut chromedriver_process, user_id, user_pw, user_number).await?;

    // 자식 요소 ID, DashMap 에 저장
    let map = match get_children_ids_to_map(&client_arc, "mf_wfm_layout_ui_generator").await {
        Ok(map) => map,
        Err(e) => return error_response("Could not get children IDs to map", e),
    };

    // data from parent_id -> vec
    let mut data_vec = match parse_data_from_parent_ids(&client_arc, map).await {
        Ok(vec) => vec,
        Err(e) => return error_response("Could not parse data from parent IDs", e),
    };

    // data_vec 정렬 및 reference_date 설정
    if data_vec.len() >= 12 {
        data_vec.sort_by(|a, b| b.claim_date.cmp(&a.claim_date));
    }

    let reference_date = data_vec.last()
        .and_then(|d| d.claim_date)
        .map(|date| format!("{}년 {:02}월", date.year(), date.month()))
        .unwrap_or_else(|| "N/A".to_string());

    // 뒤로 가기
    if let Err(e) = client_arc.back().await {
        return error_response("Failed to navigate back", e); // 에러 메시지 단순화
    }

    // select 로드 대기 및 옵션 인덱스 검색
    let select_locator = Locator::Id("mf_wfm_layout_slb_searchYm_input_0");
    wait_for_element(&client_arc, select_locator, &mut chromedriver_process).await?;
    let option_index = get_option_index(&client_arc, select_locator, &reference_date).await?;
...
// 오류 응답을 생성하는 헬퍼 함수 추가
fn error_response(message: &str, error: impl std::fmt::Debug) -> impl IntoResponse {
    (
...
// 네비게이션 및 클릭을 처리하는 함수로 코드 중복 제거
async fn handle_navigation_and_click(
    client: &Arc<Client>,
    ...
    // menu button 로드 대기 및 클릭
    wait_for_element(client, Locator::Id("mf_wfm_header_gnb_btnSiteMap"), chromedriver_process).await?;
    click_element(client, Locator::Id("mf_wfm_header_gnb_btnSiteMap")).await?;

    // login form 로드 대기 및 클릭
    wait_for_element(client, Locator::Id("mf_wfm_header_gnb_mobileGoLogin"), chromedriver_process).await?;
    click_element(client, Locator::Id("mf_wfm_header_gnb_mobileGoLogin")).await?;

    // id, pw 입력 및 로그인
    wait_for_element(client, Locator::Id("mf_wfm_header_gnb_login_popup_wframe_ui_id"), chromedriver_process).await?;
    enter_value_in_element(client, Locator::Id("mf_wfm_layout_slb_searchYm_input_0"), user_id).await?;
    enter_value_in_element(client, Locator::Id("mf_wfm_layout_slb_searchYm_input_0"), user_pw).await?;
    click_element(client, Locator::Id("mf_wfm_layout_slb_searchYm_input_0")).await?;

    // 요금 조회 버튼 클릭
    click_element_with_retries(client, Locator::XPath("/html/body/div[2]/div[3]/div/div/div[4]/div/div[2]/div[1]/a[3]"), 10).await?;

    // 필드 로드 대기 및 고객 번호 입력
    wait_for_element(client, Locator::Id("mf_wfm_layout_inp_searchCustNo"), chromedriver_process).await?;
    ...
    // 상세 요금 버튼 로드 대기 및 클릭
    wait_for_element(client, Locator::Id("mf_wfm_layout_ui_generator_0_btn_moveDetail"), chromedriver_process).await?;
    ...
    // 로딩 대기
    wait_for_element_hidden(client, Locator::Id("mf_wq_uuid_1_wq_processMsgComp"), chromedriver_process, Duration::from_secs(20)).await?;
